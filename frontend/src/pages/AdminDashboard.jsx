...
export default function AdminDashboard() {
  ...
  const [formTarget, setFormTarget] = useState(null)   // null = closed, 'new' = create, field obj = edit
    ...
    <Layout>
      <div className="max-w-6xl mx-auto px-6 py-8 flex flex-col gap-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">Manage fields, agents and monitor season progress</p>
          </div>
          <button
            onClick={() => setFormTarget('new')}
            className="flex items-center gap-2 bg-[#3d6b35] text-white text-sm font-semibold px-4 py-2.5 rounded-xl hover:bg-[#2f5429] transition cursor-pointer shadow-sm"
          >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            <div key={label} className={`rounded-2xl border px-5 py-4 flex items-center gap-4 ${style}`}>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 text-xs text-gray-400 font-semibold uppercase tracking-wide">
                  ...
                  <th className="text-left px-5 py-3 hidden sm:table-cell">Stage</th>
                  <th className="text-left px-5 py-3">Status</th>
                  <th className="text-left px-5 py-3 hidden md:table-cell">Agent</th>
                  <th className="text-left px-5 py-3 hidden lg:table-cell">Last Updated</th>
                ...
                {fields.map((field) => {
                  ...
                  return (
                    <tr key={field.id} className="border-b border-gray-50 hover:bg-[#f4f6f3] transition">
                      <td className="px-5 py-3 font-medium text-gray-800 cursor-pointer hover:text-[#3d6b35]" onClick={() => setViewing(field)}>
                        {field.name}
                      </td>
                      <td className="px-5 py-3 text-gray-500">{field.crop}</td>
                      <td className="px-5 py-3 text-gray-500 hidden sm:table-cell">{field.stage}</td>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {agents.map((agent) => {
              ...
              return (
                <div key={agent.id} className="bg-white rounded-2xl border border-gray-100 px-5 py-4 flex items-center gap-4">
                  <div className="w-10 h-10 rounded-full bg-[#3d6b35]/10 flex items-center justify-center text-[#3d6b35] font-bold text-sm shrink-0">
                    {agent.username[0].toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-gray-700 text-sm">{agent.username}</p>
      ...
      {formTarget && (
        <FieldFormModal
          ...
          agents={agents}
          onSave={(data) => formTarget === 'new' ? createField(data) : updateField(formTarget.id, data)}
