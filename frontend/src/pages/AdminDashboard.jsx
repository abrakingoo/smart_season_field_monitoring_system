...
export default function AdminDashboard() {
  ...
  const statCards = [
    { label: 'Total Fields', key: null,        value: fields.length,          style: 'bg-white border-gray-100',                        icon: <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 text-[#3d6b35]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg> },
    { label: 'Active',       key: 'Active',    value: counts.Active || 0,     style: 'bg-emerald-50 border-emerald-100 text-emerald-700', icon: <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg> },
    { label: 'At Risk',      key: 'At Risk',   value: counts['At Risk'] || 0, style: 'bg-amber-50 border-amber-100 text-amber-700',      icon: <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> },
          ...
          <>
            {section === 'overview' && (
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                {statCards.map(({ label, key, value, style, icon }) => (
                  <button
                    key={label}
                    ...
                    className={`rounded-2xl border px-3 sm:px-5 py-3 sm:py-4 flex items-center gap-2 sm:gap-4 text-left transition cursor-pointer ${style} ${
                      statusFilter === key ? 'ring-2 ring-offset-1 ring-current shadow-md scale-[1.02]' : 'opacity-80 hover:opacity-100 hover:shadow-sm'
                    }`}
                  >
        ...
        {(section === 'overview' || section === 'fields') && (
          <div>
            <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-widest mb-3">
              {statusFilter ? `${statusFilter} Fields (${filteredFields.length})` : `All Fields (${fields.length})`}
            </p>
